# IT Documentation & Knowledge Base

A collection of IT helpdesk documentation, procedures, and guides for end-users and IT support staff.

![Documentation](https://img.shields.io/badge/Documentation-IT%20Support-blue)
![Status](https://img.shields.io/badge/Status-Complete-green)

## 📋 Overview

This repository contains standardized IT documentation covering common helpdesk procedures, troubleshooting guides, and user-facing instructions. Designed for IT support teams to reduce ticket volume and provide consistent service.

## 📁 Contents

### For IT Staff

| Document | Description |
|----------|-------------|
| [What is a Helpdesk Ticket?](/what-is-a-ticket.md) | Understanding tickets, priorities, and SLAs |
| [Common Issues & Quick Fixes](/common-issues-quick-fixes.md) | Top 10 helpdesk problems and solutions |
| [Escalation Guide](/escalation-guide.md) | When and how to escalate to Tier 2/3 |
| [New Employee IT Setup](/new-employee-setup.md) | Onboarding checklist |
| [Employee Offboarding](/offboarding-checklist.md) | Account deactivation procedure |
| [IT Terminology Glossary](/it-glossary.md) | Common terms and definitions |

### For End Users

| Document | Description |
|----------|-------------|
| [Password Reset Guide](/password-reset.md) | Self-service and IT-assisted resets |
| [VPN Troubleshooting](/vpn-troubleshooting.md) | Remote connection issues |
| [Email & Outlook Issues](/outlook-troubleshooting.md) | Common email problems |
| [How to Submit an IT Ticket](/how-to-submit-ticket.md) | Getting help from IT |

### Visual Guides

| Document | Description |
|----------|-------------|
| [Troubleshooting Flowchart](/troubleshooting-flowchart.md) | Decision tree for common issues |

## 🎯 Purpose

- **Reduce ticket volume** — Users can self-service common issues
- **Consistent support** — Standardized procedures for IT staff
- **Faster resolution** — Quick reference for common problems
- **Training resource** — Onboard new IT staff quickly

## 🚀 Quick Start

1. Browse the `docs/` folder
2. Find the relevant guide
3. Follow step-by-step instructions
4. Contact IT if issue persists

## 📊 Common Issues by Frequency

| Issue | Frequency | Self-Service? |
|-------|-----------|---------------|
| Password Reset | 🔥 Very High | ✅ Yes |
| Account Locked | 🔥 Very High | ✅ Yes |
| VPN Connection | High | Partial |
| Email/Outlook | High | Partial |
| Printer Issues | Medium | ❌ Contact IT |
| Software Install | Medium | ❌ Contact IT |
| Slow Computer | Medium | Partial |

## 📞 Contact IT

- **Email:** helpdesk@company.com
- **Portal:** https://helpdesk.company.com
- **Phone:** x1234 (internal)
- **Hours:** Monday-Friday, 8 AM - 6 PM

## 👤 Author

**Amogh Karankal**

- GitHub: [@Amogh-Karankal](https://github.com/Amogh-Karankal)
- LinkedIn: [Amogh Karankal](https://www.linkedin.com/in/amoghkarankal/)

## 📄 License

This project is licensed under the MIT License.

---

*Last Updated: March 2026*
